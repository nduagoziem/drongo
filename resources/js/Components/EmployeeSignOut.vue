...
const signOut = async () => {
    ...
    if (result.state === "granted") {
        ...
        form.signOutTime = currentTime.value;

        try {
            const response = await axios.post(route("attendance.signOut", props.user_uid), {
...
<template>
    <div class="flex justify-center mt-6 py-12">
        <form action="" @submit.prevent class="w-full">
            ...
            <label id="time" class="block text-sm font-medium text-gray-700">GoodBye, See you next time</label>
